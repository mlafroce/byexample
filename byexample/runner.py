...
class PexpectMixin(object):
    ...
    def _expect_prompt_or_type(
        ...
        while i < len(input_list):
            ...
            # comes from the same line (TODO: this is not takeing into
            # consideration an example like this:
            #   >>> input("very long string here:")
            #   very long string here: [foo]
            #
            # Because "very long string here:" is very long, the prefix will
            # be shorter like "string here:". The first part will be added
            # to the last_output in one chunck and the rest "string here: [foo]"
            # to the other! TODO!
            chunk = "{}[{}]\n".format(self.interpreter.match.group(), input)
            self.last_output.append(chunk)
